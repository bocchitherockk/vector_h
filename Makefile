...
preprocess: Makefile
	...
	@for file in $(preprocess_files); do \
		...
		mkdir -p "$$dirname"; \
		$(CC) -E "$$file" -o "$(preprocess_dir)/$$(dirname "$$file")/$$(basename "$$file" .c).i"; \
	done
...
assembly: Makefile
	...
	@for file in $(assembly_files); do \
		...
		mkdir -p "$$dirname"; \
		$(CC) -S "$$file" -o "$$dirname/$$(basename "$$file" .c).s"; \
	done
...
object: Makefile
	...
	@for file in $(object_files); do \
		...
		mkdir -p "$$dirname"; \
		$(CC) -c "$$file" -o "$$dirname/$$(basename "$$file" .c).o"; \
	done
...
link: object Makefile
	...
	@$(CC) $(link_object_files) -o $(executable_dir)/$(executable_file)
...
############ static library ############
static_lib_name = libmylibrary.a
static: object
	@mkdir -p $(lib_path)
	@if [ $(verbose_mode) ]; then \
...
shared_lib_name = libmylibrary.so
shared: object
	@mkdir -p $(lib_path)
	@if [ $(verbose_mode) ]; then \
		echo "Created directory: $(lib_path)"; \
